...
public class Assignment1 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < keyLength; i++) {
			...
			FrequencyAnalysis.displayMap(freq);
			char e = FrequencyAnalysis.highestCount(freq);
			char k = FrequencyAnalysis.distanceToE(e);
			System.out.println("Most common '" + e + "', key char '" + k + "'");
			key += k;
		}
		...
		System.out.println(FrequencyAnalysis.distanceToE('o'));
		
		cipher = new VigenereCipher("wkc");
		System.out.println(cipher.decrypt(ciphertext));
		
		cipher = new VigenereCipher("hallo");
		System.out.println(cipher.decrypt(cipher.encrypt("plaintext is nu weer leesbaar")));
	}
}